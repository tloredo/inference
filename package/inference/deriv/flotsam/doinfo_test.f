...
c+++  Local variables:
        integer npmax, npar
        real*8 twopi
        parameter (npmax = 15, twopi = 2.d0*3.141592653589793d0)
        real*8 info(npmax,npmax), L(npmax,npmax), vec(npmax)
        real*8 ierr(npmax,npmax)
        real*8 evecs(npmax,npmax), evals(npmax), w1(npmax), w2(npmax)
        integer order(npmax)
        real*8 nfunc
        external nfunc
...
c---  extrapolation steps.
        call ninfol2(lpp_tv, npar, mode, lpjmax, dp, nrstp, 
     *               info, ierr, npmax)
