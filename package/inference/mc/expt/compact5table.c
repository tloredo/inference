 ...
 *
 * Altered for fpsample by TJL:  Removed handling of randomkit state;
 * save more explicit table info.
 */
...
Sampler* init_sampler5tbl(double* weights, unsigned long n)
{
    ...
    else
    {
        ...
        sampler->sizeAA = sizeAA;
        sampler->sizeBB = sizeBB;
        sampler->sizeCC = sizeCC;
        sampler->sizeDD = sizeDD;
 ...
 * sampler */
unsigned long Dran(Sampler* sampler, rk_state* rng_state)
{
    uint32_t j;
    if (sampler->prob1event > -1)
    {
