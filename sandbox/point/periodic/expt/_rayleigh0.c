...
rsgrid (PyObject *self, PyObject *args) {
    PyObject *input;
    PyArrayObject *data, *P;
    int ndat, nw, i, j, Pdims[1];
    double *times, *Pdata;
    double *sd, *cd1, *swt, *cwt;
    double w_l, w_u, dw, dwt, wt, temp;
    double S, C;
...
/** Clean up and return everything in a tuple. */
    ...
    free(swt);
    free(cwt);
    return Py_BuildValue("N", PyArray_Return(P));
...
"lmargwk(k,n,S):  Calculate the log marginal likelihood for k and w.";
