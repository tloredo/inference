...
C   ... This is now handled separately if needed.
C      *  SUBROUTINE KMDIF IS CALLED IF THE USER HAS REQUESTED A            *
C      *  DIFFERENTIAL KM ESTIMATOR.                                        *

C       IF(KDIFF .EQ. 1) THEN
C          CALL KMDIF(SX,ZXU,BWRK1,BWRK2,BWRK3,WRK1,NTOT,START,
C     +               BINSIZ,LSTEP,IXU)
C       ENDIF
...
C      **********************************************************************
C
       SUBROUTINE KMDIF(IU,S,ZU,NTOT,START,BINSIZ,LSTEP,BS,BL,DIFF,F)
...
cf2py intent(in) IU, S, ZU, NTOT
cf2py intent(in) START, BINSIZ, LSTEP
cf2py intent(out) BS, BL, DIFF
cf2py intent(cache,hide) F

       IMPLICIT REAL*8 (A-H,O-Z), INTEGER (I-N)

       DIMENSION ZU(IU),S(IU),F(IU)
